...
cartController.addToCart = async (req, res) => {
  ...
  if(user) {
    ...
    if(cart == null) {
      ...
      await Product.decrement("stockProd", { by: quantityProd, where: { idProd: idProd } })
        
      var total = await CartDetail.findOne({
        attributes: [[sequelize.fn("SUM", sequelize.literal("quantity * unit_price")), "total"]]
      }, {
        where: {
      ...
      if(detail == null) {
        ...
        await detail.increment("quantity", { by: quantityProd, where: { CartIdCart: cart.idCart } });
        await Product.decrement("stockProd", { by: quantityProd, where: { idProd: idProd } })
      }
